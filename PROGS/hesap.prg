...
FUNCTION getAsil_Alacak
	...
	**************************************************
	* Takip Öncesi Tahsilat Varsa Asýl Alacaktan Düþ
	**************************************************
	lnTahsilat_Tutari = 0
	...
	*debugmode()
	...
	WITH loAlacakBizObj
		...
		DO WHILE lnResult = File_Ok OR lnResult = File_Bof
			...
			llReqResult = .F.
			WITH loFaizBizObj
				.setparameter("vp_FaizOrani_Fk", lcAlacak_Id) && Özel Faiz Oraný tanýmlanmýþ olabilir
				...
				IF NOT llReqResult && Dosya Tipindeki faiz Tanýmýna bak
				...
				FOR i = 1 TO lnRow
					...
					*.Faiz_Aciklamasi = .Faiz_Aciklamasi + DTOC(ldFaiz_Baslama_Tarihi) + " itibaren %"
					...
					DO CASE
						...
						OTHERWISE
							IF lcHesap_Tipi = "Ýndirimli Hesap"
...
FUNCTION getVekaletUcreti
	...
	*debugmode()
	...
	WITH loVekaletBizObj
		.setparameter("vp_VekaletUcreti_Fk", tcIcraDosya_Id)   && Dosyaya özel Vekalet ücreti tanýmlanmýþ mý
		IF .REQUERY() = Requery_Success AND .recordcount > 0
			lnResult = .NAVIGATE("FIRST") && Assume find latest date
		ELSE
			.setparameter("vp_VekaletUcreti_Fk", tcDosyaTipi_Id)  && Dosya Tipine özel vekalet Ücreti Tanýmlanmýþ mý
			IF .REQUERY() = Requery_Success AND .recordcount > 0
				lnResult = .NAVIGATE("FIRST") && Assume find latest date
			ELSE
				.resetparameters()
				.setparameter("vp_Vekalet_Ucreti_Tipi", "Resmi Vekalet Ücreti")   && Resmi Vekalet Ücretini kullan
				IF .REQUERY() = Requery_Success AND .recordcount > 0
					lnResult = .NAVIGATE("FIRST") && Assume find latest date
				ENDIF
			ENDIF
...
*debugmode()
	...
	*!*		llNewBuro = .F.
	*!*		WITH loApp
	*!*			loBuroBizObj    = .Getbizobj("BuroViewBizObj")
	*!*		ENDWITH
	*!*		IF VARTYPE(loBuroBizObj) = T_Object AND TYPE([loBuroBizObj.Name]) = T_Character
	*!*		ELSE
	*!*			llNewBuro	 = .T.
	*!*			loBuroBizObj = CREATEOBJECT("BuroViewBizObj")
	*!*		ENDIF
	...
	WITH loVekilBizobj
				...
				IF .REQUERY() = Requery_Success AND .recordcount > 0
					...
					WITH loPersRec
						lcVekil_Bilgisi = lcVekil_Bilgisi + ;
							...
							N2B(.Vergi_Dairesi) + " " + N2B(.Vergi_No) + " " + N2B(.Ssk_No) + " \par "
					ENDWITH
				ENDIF
			ENDWITH
			lnOk = .NAVIGATE("next")
		...
		*!*					WITH loBuroRec
		*!*						lcVekil_Bilgisi = lcVekil_Bilgisi + ;
		*!*							N2B(.Buro_adi) + " " + N2B(.Adres) + " " + N2B(.Il_adi) + " " + N2B(.Ilce_Adi) + " \par "
	...
	WITH loHarcBizObj
		...
		DO WHILE lnOk = File_Ok OR lnOk = File_Bof
			...
			*!*				ENDIF
			...
			IF ALLTRIM(lcHarc_Tipi) = "Peþin Harç"
				lnPesin_Harc = lnHarc_Miktari
			ENDIF

			IF INLIST(ALLTRIM(lcHarc_Tipi), "Haciz Öncesi Tahsil Harcý", "Tahsilat Harcý")
		...
		m.Tahsil_Harci = lnTahsil_Harci - lnPesin_Harc  && m.Tahsil_Harci Private deðiþken

		lnHarc_Tutari = .CalculateSum([Harc_Miktari For Harc_Tipi<>'Haciz Öncesi Tahsil Harcý' And Harc_Tipi<>'Tahsilat Harcý']) && Tahsilat Harci ayrý bir kalem halinde hesaplanýr
...
FUNCTION GetTakipAlacakHesabi
	...
	lcTakip_Alacak_Hesabi = ;
		...
		"-------------- " + "\par " + ;
		TRANSFORM(m.Takip_Alacagi, "999,999,999.99") + " Takip Alacaðý" &&+"\par"
...
FUNCTION GetToplamAlacakHesabi
	...
	lcToplam_Alacak_Hesabi = ;
		ALLTRIM(TRANSFORM(m.Takip_Alacagi, "999,999,999.99")) + " TL Takip Alacaðý + " + ;
		ALLTRIM(TRANSFORM(m.Ts_Vekalet_Ucreti, "999,999,999.99")) + " TL Vekalet Ücreti + " + ;
		ALLTRIM(TRANSFORM(m.Ts_Harc_Tutari, "999,999,999.99")) + " TL Diðer Harç Tutarý + " + ;
		ALLTRIM(TRANSFORM(m.Ts_Masraf_Tutari, "999,999,999.99")) + " TL Masraf Tutarý + " + ;
		ALLTRIM(TRANSFORM(m.Tahsil_Harci, "999,999,999.99")) + " TL Tahsil_Harcý + " + ;
		ALLTRIM(TRANSFORM(m.Takip_Faizi_Tutari, "999,999,999.99")) + " TL Takip Faizi Tutarý" + ;
