...
CASE VARTYPE(tuParameter) = T_Object
	** loIcraDosyayaVal gönderilmiþ, Icradosyabizobj ye gerek yok
	loIcraDosyaVal = tuParameter
	lcIcraDosya_Id = loIcraDosyaVal.IcraDosya_Id
CASE VARTYPE(tuParameter) = T_Character
	llreturn = .F.

	*!*			lcIcraDosya_Id=tuParameter
	*!*			loIcraDosyaBizObj=loApp.GetbizObj("IcraDosyaEditBizObj")
	*!*			IF VARTYPE(loIcraDosyaBizObj)=T_Object AND TYPE([loIcraDosyaBizObj.Name])=T_Character
	*!*			ELSE
	*!*				loIcraDosyaBizObj=CREATEOBJECT("IcraDosyaEditBizObj")
	*!*			ENDIF
	*!*			WITH loIcraDosyaBizObj
	*!*				.setparameter("IcraDosya_Id",lcIcraDosya_Id)
	*!*				IF REQUERY()=REQUERY_SUCCESS AND. recordCount>0
	*!*					loIcraDosyaVal=.GetDEValues()
	*!*				ELSE
	*!*					llreturn=.F.
	*!*				ENDIF
	*!*			ENDWITH
	...
	*!*					.setparameter("vp_IcraDosyaHesabi_Fk",lcIcraDosya_Id)
	*!*					.REQUERY()
	*!*				ENDWITH
	*!*			ENDIF
	*!*			WITH loIcraDosyaHesabiBizObj
	*!*				loIcraDosyaHesabiRec=.GetValues()
	*!*			ENDWITH
...
*!*		IF NullOrEmpty(.Hesap_Tipi)
*!*			.Hesap_Tipi = "Standart Hesap"
*!*		ENDIF

*!*		IF .Hesap_Tipi = "Özel Hesap" OR ;
*!*				.Hesap_Tipi = "Kampanya Hesabý"
*!*			RETURN .T.
*!*		ENDIF
*!*	ENDWITH
...
m.Tahsil_Harci	 = ROUND(m.Tahsil_Harci, 2) && m.Tahsil_Harci Private deðiþken... Get Harc Tutari fonsiyonunda hesaplaniyor
...
**** Takip Düþme Sýrasý ve Toplam Alacak hesabý
...
***** Kalan Alacak Hesabý
m.Kalan_Alacak = m.Toplam_Alacak - m.Ts_Tahsilat_Tutari  && Indirim Tutarý ikinci hesapta düþülecek
