...
#DEFINE FBC_CODE_GS1128		159 && GSI-128
...
#DEFINE FBC_CODE_MSIPLESSEY	172 && MSI/Plessey
...
#DEFINE FBC_CODE_POSTNET	180 && PostNet
#DEFINE FBC_CODE_IMB		181 && Intelligent Mail Barcode
...
DEFINE CLASS FoxBarcode AS CUSTOM
    ...
    *-- Validates that the font and style are permitted by GDI+
    ...
    IF NOT EMPTY(THIS.cTextValue)
      ...
      DO CASE
        ...
        CASE THIS.nBarcodeType = FBC_CODE_MSIPLESSEY && Plessey
      ...
      *-- Barcode types
      DO CASE
        ...
        CASE THIS.nBarcodeType = FBC_CODE_IMB && Intelligent Mail Barcode
          FOR ln = 1 TO THIS.nLenPattern
            lnCol = ln - 1
            LOCAL lnFull
            lnFull = THIS.nImageHeight - 2 * THIS.nMargin
            DO CASE
              ...
              CASE SUBSTR(THIS.cPattern, ln, 1) == "D" && Desc
                THIS.oGra.DrawLine(lnCol * THIS.nFactor + THIS.nMargin, THIS.nMargin + lnFull * .33, lnCol * THIS.nFactor + THIS.nMargin, THIS.nImageHeight - THIS.nMargin)
        ...
        CASE THIS.nBarcodeType = FBC_CODE_PHARMA2 && Two Track Pharmacode
          IF THIS.lShowHumanReadableText
            lnBarFull = THIS.nImageHeight - THIS.nTextHeight - THIS.nMargin
          ELSE
            lnBarFull = THIS.nImageHeight - THIS.nMargin
          ENDIF
      ...
      IF THIS.lShowHumanReadableText
        ...
        *-- Without Human readable
        IF THIS.nBarcodeType = FBC_CODE_POSTNET OR ; && PostNet
          THIS.nBarcodeType = FBC_CODE_IMB && Intelligent Mail Barcode
          *-- No clear zone
        ELSE
          *-- Clear zone
  ...
  *------------------------------------------------------
  FUNCTION Code_IMB()
    ...
    *!*	    IF NOT CHRTRAN(THIS.cTextValue,CHRTRAN(THIS.cTextValue,lcValid,""),"") == THIS.cTextValue
    *!*	      *-- Chars or lenght not valid
    *!*	      THIS.cMsgError = "The Intelligent Mail Barcode accepts only numeric characters."
    *!*	      RETURN NULL
    *!*	    ENDIF

    *!*	    IF LEN(THIS.cTextValue) # 5 AND LEN(THIS.cTextValue) # 9 AND LEN(THIS.cTextValue) # 11
    *!*	      *-- Chars or lenght not valid
    *!*	      THIS.cMsgError = "Intelligent Mail Barcode: The length of the string must be 5,9,11"
    *!*	      RETURN NULL
    *!*	    ENDIF
    ...
    *-- Chars or lenght not valid
    ...
    THIS.cMsgError = "Intelligent Mail Barcode soon ..."
    RETURN NULL
  ...
  *------------------------------------------------------
  FUNCTION Code_Pharmacode1()
    ...
    IF NOT CHRTRAN(THIS.cTextValue,CHRTRAN(THIS.cTextValue,lcValid,""),"") == THIS.cTextValue
      *-- Chars or lenght not valid
      THIS.cMsgError = "The Pharmacode accepts only numeric characters."
      RETURN NULL
    ENDIF

    IF NOT BETWEEN(VAL(THIS.cTextValue),3,131070)
      *-- Chars or lenght not valid
      THIS.cMsgError = "Parmacode: Input values between 3 and 131070"
      RETURN NULL
    ENDIF
  ...
  *------------------------------------------------------
  FUNCTION Code_Pharmacode2()
    ...
    IF NOT BETWEEN(VAL(THIS.cTextValue),4,64570080)
      *-- Chars or lenght not valid
      THIS.cMsgError = "Two Track Parmacode: Input values between 4 and 64570080"
      RETURN NULL
    ENDIF
